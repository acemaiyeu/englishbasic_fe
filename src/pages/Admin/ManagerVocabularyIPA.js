...
class ManagerVocabularyIPA extends React.Component {
    ...
    PageVocabulary = async (page) => {
        ...
        this.setState({
         ...
         await this.getListVocabularyIPA(this.state.lesson_id_create);
    }
    refectData = () => {
        this.setState({
            loadding: true,
        ...
        this.getListVocabularyIPA(this.state.listLessons.data[0].id);
    }
    getListLessons = async () => {
        ...
        api_admin.get(`${API_URL}/admin/lessons?title_english=` + 
            ...
            if(this.state.listVocabulary.length === 0){
                this.getListVocabularyIPA(res.data?.data[0]?.id);
            }
        }).catch((err) => {
            console.log(err);
    ...
    render(){
        let { language_type, loadding, sound, vocabulary_form, transcription, means, title_english, listVocabulary } = this.state;
        return (
            <div className="data-manager-container">
                <div className="data-manager-title" >{language_type === "EN" ? "MANAGER API's VACABULARY" : "Quản lý từ vựng IPA"}</div>
                ...
                <div className="data-manager-add-vocabulary-form">
                    ...
                    <div className="data-manager-add-vocabulary-form-body">
                        ...
                        <div className="form">
                            <label> {language_type === "EN" ? "Sound" : "Âm"}: </label>
                            <input type="text" value={sound} onChange={(e) => this.handleChangeInput(e, "sound")}/>
                        </div>
...
export default connect(mapStateToProps)(ManagerVocabularyIPA);
